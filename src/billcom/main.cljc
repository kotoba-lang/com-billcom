(ns billcom.main
  "Kotodama WASM entrypoint for the Billcom clean-room actor (L4) — Clojure port.
  ...
  facts (`billcom.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "billcom")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"billcom.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "billcom-compat" :tier tier :entities entities} 200])
...
(defn start! [] :billcom-compat/ready)
